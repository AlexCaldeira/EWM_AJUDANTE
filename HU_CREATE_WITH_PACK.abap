...
CALL METHOD g_create->/scwm/if_pack_bas~create_hu
  ...
  CALL METHOD g_create->/scwm/if_pack_bas~pack_hu
    EXPORTING
      iv_source_hu = lv_source_hu-guid_hu "HU a ser packed da classe criada em cima
      iv_dest_hu   = es_hundr-guid_hu "Case criado em cima é a HU de destino
  ...
  IF sy-subrc = 0.
    CALL METHOD g_create->/scwm/if_pack_bas~base
      EXPORTING
        iv_commit = 'X'
        iv_wait = 'X'
      EXCEPTIONS
